...
from .indic_en import (get_dictionary_for, get_vowels_for,
                       get_vowel_signs_for, get_virama_for, get_anuswara_for)
...
class Transliterator:
    """
    Transliteration class, instantiate this to get access  to the
    ...
    def transliterate_en_xx(self, word, target_lang):
        ...
        :param word: The word to be transliterated.
        :type word: str.
        :param target_lang: The language into which word has to be
        transliterated.
    ...
    def transliterate_iso15919(self, word, src_language):
        ...
        for character in word:
            index += 1
            offset = ord(character) - lang_bases[src_language]
            ...
            if tx_str[-1:] == 'a' and (src_language == "hi_IN" or
                                       src_language == "gu_IN" or
                                       src_language == "bn_IN"):
                if word_length == index and word_length > 1:  # if last letter
                    tx_str = tx_str[:-1]  # remove the last 'a'
        try:
            result = tx_str.decode("utf-8")  # noqa: F821
    ...
    def transliterate_ipa(self, word, src_language):
        ...
        for character in word:
            index += 1
            if ord(character) < 255:  # ASCII characters + English
                tx_str += character
                continue
            offset = ord(character) - lang_bases[src_language]
            # 76 is the virama offset for all indian languages from its base
            ...
            # delete the inherent 'a' at the end of the word from hindi
            if tx_str[-1:] == 'ə' and \
               (src_language == "hi_IN" or
                src_language == "gu_IN" or
                src_language == "bn_IN") and \
               (word_length == index and
                    word_length > 1):
                tx_str = tx_str[:-(len('ə'))]
            # if last letter
            # remove the last 'a'
    ...
    def transliterate_indic_indic(self, word, src_lang, target_lang):
        ...
        for character in word:
            index += 1
            if character in string.punctuation or (ord(character) <= 2304 and
                                                   ord(character) >= 3071):
            ...
            if(offset > 0):
                try:
                    tx_str = tx_str + unichr(offset)  # noqa: F821
                except BaseException:
                    tx_str = tx_str + chr(offset)
            # schwa deletion
            baseoffset = offset - lang_bases[target_lang]
            # 76 : virama
            if (index == len(word) and baseoffset == 76 and
                    (target_lang == "hi_IN" or
                     target_lang == "gu_IN" or
        ...
        if ((target_lang == "ml_IN") and
            (src_lang == "hi_IN" or
             src_lang == "gu_IN" or
             src_lang == "pa_IN" or
             src_lang == "bn_IN") and tx_str[-1].isalpha()):
            tx_str = tx_str + u"്"
        return tx_str
    ...
    def transliterate_indic_en(self, word, src_lang):
        ...
        while index < word_length:
            ...
            if index + 1 < word_length and \
                    not word[index + 1] in vowel_signs and \
                    word[index + 1] in dictionary and \
                    not word[index] in vowels and \
                    not word[index] in vowel_signs:
                tx_string += 'a'

            if index + 1 == word_length and \
                    not word[index] in vowel_signs and \
                    word[index] in dictionary:
                tx_string += 'a'

            # handle am sign
            if index + 1 < word_length and word[index + 1] == anuswara and \
                    not word[index] in vowel_signs:
                tx_string += 'a'
            index += 1
        return tx_string
    ...
    def transliterate(self, text, target_lang_code):
        ...
        :type text: str.
        :param target_lang_code: The language into which word has to be
        transliterated.
